...
export default function DashboardEnrollmentRequestsPage() {
  ...
  async function handleDecision(request, decision) {
    ...
    try {
      await api.post(
        `/schools/${schoolId}/enrollment-requests/${request.id}/${decision}`,
        payload,
      );
      await reload();
    } catch (err) {
      setActionError(
        err.response?.data?.message || "Impossible de traiter cette demande.",
      );
    } finally {
      setActingId(null);
    }
  ...
  function openAdmission(request) {
    ...
    setAdmissionForm({
      ...
      child_birthdate: request.child_birthdate
        ? request.child_birthdate.slice(0, 10)
        : "",
      gender: "",
    ...
    try {
      ...
      setActionError(
        messages
          ...
          : err.response?.data?.message || "Impossible d’admettre cet élève.",
      );
    } finally {
      setActingId(null);
    }
  ...
  return (
    <Box>
      ...
      <Typography color="text.secondary" sx={{ mb: 3 }}>
        Demandes de pré-inscription envoyées depuis la homepage par des parents
        ou élèves.
      </Typography>
            ...
            >
              <Box sx={{ flexGrow: 1, minWidth: 220 }}>
                <Typography variant="subtitle2">
                  {r.child_fullname}
                  {r.level?.name
                    ? ` — ${r.level.section?.name ? `${r.level.section.name} · ` : ""}${r.level.name}`
                    : ""}
                </Typography>
              ...
              <Chip
                label={STATUS_LABELS[r.status].label}
      ...
      <Dialog
        open={Boolean(admissionRequest)}
        onClose={() => setAdmissionRequest(null)}
        fullWidth
        maxWidth="sm"
      >
        <Box component="form" onSubmit={confirmAdmission}>
          ...
          >
            ...
            <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
              <TextField
                ...
                type="date"
                value={admissionForm.child_birthdate}
                onChange={(event) =>
                  setAdmissionForm((form) => ({
                    ...form,
                    child_birthdate: event.target.value,
                  }))
                }
            ...
            <TextField
              ...
              helperText="Requis seulement si le parent n’a pas déjà un compte avec ce téléphone."
              fullWidth
            />
            <TextField
              select
              label="Lien de parenté"
              value={admissionForm.parent_relationship}
              onChange={(event) =>
                setAdmissionForm((form) => ({
                  ...form,
          ...
          <DialogActions sx={{ px: 3, pb: 2 }}>
            ...
            <Button
              ...
              variant="contained"
              disabled={
                actingId === admissionRequest?.id || classes.length === 0
              }
            >
              {actingId === admissionRequest?.id
                ? "Admission..."
                : "Créer l’inscription"}
            </Button>
          </DialogActions>
