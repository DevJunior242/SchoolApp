...
export default function DashboardBusesPage() {
  ...
  const driverList = asArray(drivers).filter(
    (m) => m.role?.slug === "chauffeur",
  );
  ...
  async function handleSubmitStops(e) {
    ...
    try {
      ...
      setStopsError(
        messages
          ? Object.values(messages).flat().join(" ")
          : "Impossible d'enregistrer les arrêts.",
      );
    } finally {
        ...
        <Stack spacing={1.5} sx={{ mb: 3 }}>
          {busList.map((bus) => (
            <Card
              ...
              sx={{
                borderColor:
                  selectedBusId === bus.id ? "primary.main" : undefined,
              }}
            >
              <CardContent
                sx={{
              ...
              >
                <Box sx={{ flexGrow: 1 }}>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    {bus.plate_number || "Sans plaque"} ·{" "}
                    {bus.driver
                      ? bus.driver.fullname
                      : "Aucun chauffeur assigné"}{" "}
                    · {bus.stops?.length ?? 0} arrêt(s)
                  </Typography>
                </Box>
                <IconButton
                  size="small"
      ...
      {selectedBus && (
        <Box>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Latitude/longitude optionnelles mais nécessaires pour le suivi en
            direct et l'alerte "bus proche".
          </Typography>
        ...
        <Box component="form" onSubmit={handleSubmitBus}>
          ...
          >
            ...
            <TextField
              ...
              onChange={(e) =>
                setForm((p) => ({ ...p, driver_id: e.target.value }))
              }
              fullWidth
              helperText={
                driverList.length === 0
                  ? "Ajoutez d'abord un membre avec le rôle chauffeur"
                  : ""
              }
            >
