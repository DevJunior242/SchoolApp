...
export default function BecomeProviderPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (!form.phone.trim() && !form.email.trim()) {
      setError(
        "Indiquez au moins un numéro de téléphone ou un email pour être recontacté.",
      );
      return;
    }
    ...
    try {
      ...
      if (err.response?.status === 403) {
        setError(
          "Vous devez vérifier votre email avant de vous inscrire comme prestataire. Consultez votre boîte mail.",
        );
        return;
      }
  ...
  return (
    <Container maxWidth="sm" sx={{ py: { xs: 5, md: 8 } }}>
      ...
      {!user ? (
        ...
        >
          <Typography color="text.secondary" sx={{ mb: 3 }}>
            Connectez-vous ou créez un compte pour vous inscrire comme
            prestataire.
          </Typography>
        ...
        <Alert severity="warning">
          Ce compte est déjà rattaché à une école
          {user.role ? " ou à un rôle d'administration" : ""} : un prestataire
          doit utiliser un compte différent (rien à voir avec votre compte
          actuel), pour ne pas perdre votre rôle existant.
        </Alert>
      ) : (
        <Paper
          variant="outlined"
