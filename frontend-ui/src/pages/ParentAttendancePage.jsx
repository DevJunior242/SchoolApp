...
export default function ParentAttendancePage() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((err) =>
        setLoadError(
          err.response?.data?.message || "Impossible de charger vos enfants.",
        ),
      )
  ...
  async function loadAttendances(childId) {
    ...
    try {
      const response = await api.get(
        `/schools/${schoolId}/students/${childId}/attendances`,
      );
      setAttendances(response.data);
    } catch (err) {
      setLoadError(
        err.response?.data?.message || "Impossible de charger les absences.",
      );
    }
  }
  ...
  async function handleJustify(e) {
    ...
    try {
      await api.post(
        `/schools/${schoolId}/attendances/${justifyTarget.id}/justify`,
      ...
      setError(
        messages
          ...
          : "Impossible de soumettre cette justification.",
      );
    } finally {
      setSubmitting(false);
    }
            ...
            <CardContent sx={{ display: "flex", alignItems: "center", gap: 2 }}>
              <Box sx={{ flexGrow: 1 }}>
                <Typography variant="subtitle2">
                  {a?.class_subject_teacher?.subject?.name} ·{" "}
                  {a?.class_subject_teacher?.school_class?.name}
                </Typography>
              ...
              <Chip
                label={STATUS_LABELS[a.status]?.label}
                color={STATUS_LABELS[a.status]?.color}
                size="small"
              />
              {a.status !== 1 && (
                <Chip
                  label={JUSTIFICATION_LABELS[a.justification_status]?.label}
                  color={JUSTIFICATION_LABELS[a.justification_status]?.color}
                  size="small"
                  variant="outlined"
                />
              )}
              {a.status !== 1 &&
                (a.justification_status === 0 ||
                  a.justification_status === 3) && (
