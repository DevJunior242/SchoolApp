  ...
  return (
    <Box>
      ...
      <Typography color="text.secondary" sx={{ mb: 3 }}>
        Une fiche apparaît dans l'annuaire des écoles seulement si elle est
        approuvée ET que l'abonnement est à jour.
      </Typography>
...
function PlansSection() {
  const {
    ...
    error,
    reload,
  } = useApiGet("/admin/marketplace/plans");
    ...
    try {
      ...
      setFormError(
        messages
          ...
          : "Impossible de créer cette formule.",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  async function handleToggleActive(plan) {
    await api.put(`/admin/marketplace/plans/${plan.id}`, {
      active: !plan.active,
    });
    await reload();
  }
...
function PaymentMethodsSection() {
  const {
    ...
    error,
    reload,
  } = useApiGet("/admin/marketplace/payment-methods");
  const [dialogOpen, setDialogOpen] = useState(false);
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setFormError(
        messages
          ? Object.values(messages).flat().join(" ")
          : "Impossible de créer ce moyen de paiement.",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  async function handleToggleActive(method) {
    await api.put(`/admin/marketplace/payment-methods/${method.id}`, {
    ...
    <Box>
      ...
      {loading ? (
        ...
        <Stack spacing={1.5}>
          ...
          {(methods ?? []).length === 0 && (
            <Typography color="text.secondary">
              Aucun moyen de paiement défini.
            </Typography>
          )}
        </Stack>
      )}
    ...
    <Box>
      ...
      {loading ? (
        ...
        <Stack spacing={1.5}>
          ...
          {providers.length === 0 && (
            <Typography color="text.secondary">Aucune fiche ici.</Typography>
          )}
        </Stack>
      )}
    </Box>
          ...
          {payments.map((p) => (
            <Card key={p.id} variant="outlined">
              ...
              >
                <Box sx={{ flexGrow: 1, minWidth: 200 }}>
                  <Typography variant="subtitle1">
                    {p.service_provider?.business_name}
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    {Number(p.amount).toLocaleString()} · réf.{" "}
                    {p.reference || "—"} · signalé le{" "}
