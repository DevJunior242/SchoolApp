...
const DOCUMENT_TYPE_LABELS = Object.fromEntries(
  DOCUMENT_TYPE_OPTIONS.map((t) => [t.value, t.label]),
);
  ...
  return (
    <Stack spacing={1} sx={{ mb: 3 }}>
      {allergies.map((a) => (
        <Alert
          key={a.id}
          severity={a.severity === 2 ? "error" : "warning"}
...
export default function StudentHealthPage() {
  ...
  } = useApiGet(!isTeacherOnly ? base && `${base}/profile` : null);
  const { data: allergies, reload: reloadAllergies } = useApiGet(
    base && `${base}/allergies`,
  );
  const { data: vaccinations, reload: reloadVaccinations } = useApiGet(
    !isTeacherOnly ? base && `${base}/vaccinations` : null,
  );
  const { data: visits, reload: reloadVisits } = useApiGet(
    !isTeacherOnly ? base && `${base}/visits` : null,
  );
  ...
  const [profileLoadedForStudentId, setProfileLoadedForStudentId] =
    useState(null);
  ...
  async function handleAddVaccination(e) {
    ...
    Object.entries(vaccinationForm).forEach(([key, value]) =>
  ...
  async function handleDownloadVaccinationDocument(vaccination) {
    const response = await api.get(
      `${base}/vaccinations/${vaccination.id}/document`,
      { responseType: "blob" },
    );
    const ext = EXTENSION_BY_MIME[response.data.type] ?? "pdf";
    downloadBlob(
      response.data,
      `vaccin-${vaccination.name}.${ext}`.replace(/\s+/g, "-"),
    );
  }
  ...
  async function handleDownloadDocument(document_) {
    const response = await api.get(
      `${base}/documents/${document_.id}/download`,
      { responseType: "blob" },
    );
    const ext = EXTENSION_BY_MIME[response.data.type] ?? "pdf";
      ...
      <Box sx={{ maxWidth: 640, mx: "auto" }}>
        ...
        {(allergies ?? []).length === 0 ? (
          <Typography color="text.secondary">
            Aucune allergie connue pour cet élève.
          </Typography>
        ) : (
        ...
        <Box component="form" onSubmit={handleSaveProfile}>
          <Stack spacing={2} sx={{ maxWidth: 480 }}>
            <TextField
              ...
              onChange={(e) =>
                setProfileForm((p) => ({ ...p, blood_type: e.target.value }))
              }
              disabled={!canManage}
              fullWidth
            />
            ...
            <TextField
              ...
              onChange={(e) =>
                setProfileForm((p) => ({ ...p, disability: e.target.value }))
              }
              disabled={!canManage}
              multiline
              rows={2}
            ...
            <TextField
              ...
              onChange={(e) =>
                setProfileForm((p) => ({ ...p, doctor_name: e.target.value }))
              }
              disabled={!canManage}
              fullWidth
            />
                ...
                >
                  <Box sx={{ flexGrow: 1 }}>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      ...
                      {v.expires_at
                        ? ` · expire le ${formatDate(v.expires_at)}`
                        : ""}
                      {v.next_dose_at
                        ? ` · prochaine dose le ${formatDate(v.next_dose_at)}`
                        : ""}
                    </Typography>
                  </Box>
                  {v.has_document && (
                    <IconButton
                      size="small"
                      onClick={() => handleDownloadVaccinationDocument(v)}
                    >
              ...
              <Card key={v.id} variant="outlined">
                <CardContent>
                  ...
                  >
                    ...
                    {v.is_emergency && (
                      <Chip label="Urgence" size="small" color="error" />
                    )}
                    {v.rest_recommended && (
                      <Chip
                  ...
                  {v.diagnosis && (
                    <Typography variant="body2" sx={{ mt: 1 }}>
                      Diagnostic : {v.diagnosis}
                    </Typography>
                  )}
                  {v.treatment_given && (
                    <Typography variant="body2">
                      Traitement : {v.treatment_given}
                    </Typography>
                  )}
                  {v.returned_to_class_at && (
                    <Typography variant="body2">
                      Retour en classe :{" "}
                      {new Date(v.returned_to_class_at).toLocaleTimeString(
                        "fr-FR",
              ...
              <Card key={m.id} variant="outlined">
                ...
                >
                  <Box sx={{ flexGrow: 1 }}>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      {m.dosage} · du {formatDate(m.starts_on)}
                      {m.ends_on ? ` au ${formatDate(m.ends_on)}` : ""}
                    </Typography>
                    {m.notes && (
                ...
                >
                  <Box sx={{ flexGrow: 1 }}>
                    <Typography variant="subtitle1">
                      {d.label || DOCUMENT_TYPE_LABELS[d.type]}
                    </Typography>
                    <Typography variant="body2" color="text.secondary">
        ...
        <Box component="form" onSubmit={handleAddAllergy}>
          ...
          >
            <TextField
              ...
              onChange={(e) =>
                setAllergyForm((p) => ({ ...p, label: e.target.value }))
              }
              required
              fullWidth
              autoFocus
            ...
            <TextField
              ...
              onChange={(e) =>
                setAllergyForm((p) => ({ ...p, notes: e.target.value }))
              }
              multiline
              rows={2}
              fullWidth
        ...
        <Box component="form" onSubmit={handleAddVaccination}>
          ...
          >
            <TextField
              ...
              onChange={(e) =>
                setVaccinationForm((p) => ({ ...p, name: e.target.value }))
              }
              required
              fullWidth
              autoFocus
            ...
            <Button component="label" variant="outlined">
              ...
              <input
                ...
                hidden
                onChange={(e) =>
                  setVaccinationFile(e.target.files?.[0] ?? null)
                }
              />
            </Button>
            {vaccinationFile && (
              <Typography variant="body2" color="text.secondary">
                {vaccinationFile.name}
              </Typography>
            )}
          </DialogContent>
          <DialogActions sx={{ px: 3, pb: 2 }}>
            <Button onClick={() => setVaccinationDialogOpen(false)}>
              Annuler
            </Button>
            <Button type="submit" variant="contained">
              Ajouter
        ...
        <Box component="form" onSubmit={handleAddVisit}>
          ...
          >
            <TextField
              ...
              onChange={(e) =>
                setVisitForm((p) => ({ ...p, visited_at: e.target.value }))
              }
              slotProps={{ inputLabel: { shrink: true } }}
              required
              fullWidth
            ...
            <TextField
              ...
              onChange={(e) =>
                setVisitForm((p) => ({ ...p, reason: e.target.value }))
              }
              required
              fullWidth
            />
            <TextField
              ...
              onChange={(e) =>
                setVisitForm((p) => ({ ...p, diagnosis: e.target.value }))
              }
              multiline
              rows={2}
              fullWidth
            ...
            <TextField
              ...
              onChange={(e) =>
                setVisitForm((p) => ({ ...p, treatment_given: e.target.value }))
              }
              multiline
              rows={2}
              fullWidth
            ...
            <TextField
              ...
              onChange={(e) =>
                setVisitForm((p) => ({
                  ...p,
                  returned_to_class_at: e.target.value,
                }))
              }
              slotProps={{ inputLabel: { shrink: true } }}
              fullWidth
              ...
              control={
                <Checkbox
                  ...
                  onChange={(e) =>
                    setVisitForm((p) => ({
                      ...p,
                      rest_recommended: e.target.checked,
                    }))
                  }
                />
              }
        ...
        <Box component="form" onSubmit={handleAddMedication}>
          ...
          >
            <TextField
              ...
              onChange={(e) =>
                setMedicationForm((p) => ({ ...p, name: e.target.value }))
              }
              required
              fullWidth
              autoFocus
            ...
            <TextField
              ...
              onChange={(e) =>
                setMedicationForm((p) => ({ ...p, dosage: e.target.value }))
              }
              required
              fullWidth
            />
            <TextField
              ...
              onChange={(e) =>
                setMedicationForm((p) => ({ ...p, starts_on: e.target.value }))
              }
              slotProps={{ inputLabel: { shrink: true } }}
              required
              fullWidth
            ...
            <TextField
              ...
              onChange={(e) =>
                setMedicationForm((p) => ({ ...p, ends_on: e.target.value }))
              }
              slotProps={{ inputLabel: { shrink: true } }}
              fullWidth
            />
            <TextField
              ...
              onChange={(e) =>
                setMedicationForm((p) => ({ ...p, notes: e.target.value }))
              }
              multiline
              rows={2}
              fullWidth
              ...
              control={
                <Checkbox
                  ...
                  onChange={(e) =>
                    setMedicationForm((p) => ({
                      ...p,
                      parent_authorized: e.target.checked,
                    }))
                  }
                />
              }
        ...
        <Box component="form" onSubmit={handleAddDocument}>
          ...
          >
            <TextField
              ...
              onChange={(e) =>
                setDocumentForm((p) => ({ ...p, type: Number(e.target.value) }))
              }
              fullWidth
            >
            ...
            <TextField
              ...
              onChange={(e) =>
                setDocumentForm((p) => ({ ...p, label: e.target.value }))
              }
              fullWidth
            />
            <Button component="label" variant="outlined">
          ...
          <DialogActions sx={{ px: 3, pb: 2 }}>
            ...
            <Button type="submit" variant="contained" disabled={!documentFile}>
              Déposer
            </Button>
          </DialogActions>
        </Box>
